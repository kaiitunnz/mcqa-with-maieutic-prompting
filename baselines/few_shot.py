...
def main():
    log_file = open(f"time_nocot_{DATASET}.log", "w")
    err_file = open(f"time_error_nocot_{DATASET}.log", "w")
    ...
    for i, sample in tqdm(enumerate(data), file=(None if log_file is None else open(os.devnull, "w"))):
        ...
        for _ in range(MAX_RETRY_COUNT):
            ...
            if tmp.result is not None:
                matched = re.search("(true|false)$", tmp.result.lower())
                if matched:
                    result = tmp
                    break
        ...
        pred = matched.groups()[0] == "true" or matched is None
        if pred == label:
            correct += 1
        print(
            {
