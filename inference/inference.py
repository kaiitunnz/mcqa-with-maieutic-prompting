...
class InferenceWrapper:
    ...
    def infer(G: Tree):
        ...
        graph2sat, wcnf = InferenceWrapper.convert_graph_to_sat(
            node_dict, belief, consistency
        )
        ...
        if len(correct_E_dict) > 0:
            score_list, Q_consistency = InferenceWrapper.nli_with_Q(
                G["Q"].data["E"], G["Q"].data["E_tilde"], correct_E_dict
            )
        else:
            score_list = []
            Q_consistency = {}
        ...
        consistency = {
            pair: 1 if score > 0 else -1 for pair, score in consistency.items()
        } | Q_consistency
    ...
    def prepare_node_list(G: Tree) -> OrderedDict:
        ...
        for node in G.all_nodes_itr():
            if node.identifier != "Q":
                if (
                    None in node.data["blf"]
                    or node.data["blf"][0] > node.data["blf"][1]
                ):
                ...
                node_dict[identifier] = Node(
                    identifier=identifier, E=E, blf=blf, integrity=integrity
                )
    ...
    def compute_belief(node_dict: OrderedDict) -> Dict:
        ...
        for node in nodes_to_compute_blf:
            ...
            likelihood_belief = (likelihood_feature[0] - likelihood_feature[1]) / sum(
                likelihood_feature
            )
    ...
    def compute_consistency(node_dict: OrderedDict) -> Dict:
        ...
        if len(node_dict) > 1:
            ...
            all_pair_E_list = [
                (node_dict[name1].E, node_dict[name2].E)
                for name1, name2 in all_pair_list
            ]

            all_pair_E_probs = F.softmax(
                InferenceWrapper.nli_model.predict(*zip(*all_pair_E_list)), dim=-1
            )
    ...
    def nli_with_Q(Q: str, Q_tilde: str, correct_E_dict: Dict):
        ...
        E_Q_labels = (
            InferenceWrapper.nli_model.predict(E_list, [Q] * len(E_list))
            .argsort(dim=-1, descending=True)
            .tolist()
        )
        E_Q_tilde_labels = (
            InferenceWrapper.nli_model.predict(E_list, [Q_tilde] * len(E_list))
            .argsort(dim=-1, descending=True)
            .tolist()
        )

        score_list = []
        for node_name, E_Q_label, E_Q_tilde_label in zip(
            name_list, E_Q_labels, E_Q_tilde_labels
        ):
            score = 0
            if E_Q_label[0] == 2:
                score += 1
        ...
        Q_consistency = {
            (node_name, "Q"): score if abs(score) <= 1 else score / abs(score)
            for node_name, score in score_list
            if score != 0
        }
    ...
    def convert_graph_to_sat(node_dict: OrderedDict, belief: Dict, consistency: Dict):
        ...
        graph2sat = {
            node_name: idx + 1 for idx, node_name in enumerate(node_dict.keys())
        }
        ...
        for (name1, name2), weight in consistency.items():
            clause1 = (
                [-graph2sat[name1], graph2sat[name2]]
                if weight > 0
                else [-graph2sat[name1], -graph2sat[name2]]
            )
